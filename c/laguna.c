...
typedef struct {
    ...
    int model_type; // 0=GLM, 1=HY3, 2=LAGUNA
    int *head_counts; // per-layer head counts (allocated to n_layers)
    int *kv_head_counts; // per-layer KV head counts (usually constant 8 for Laguna)
    char *layer_types; // per-layer attention types: 0=global, 1=sliding_window
    char *mlp_layer_types; // per-layer MLP types: 0=dense, 1=sparse(MoE)
    float rope_theta_global; // for global attention layers
    float rope_theta_local; // for sliding window layers
    float rope_factor; // YaRN factor
    float rope_pct; // partial rotary percentage
    float yarn_beta_slow; // YaRN beta_slow parameter
    float yarn_beta_fast; // YaRN beta_fast parameter
    float attention_factor; // YaRN attention scaling factor
    float router_scaling_factor; // for softplus per-head router
} Cfg;
...
/* YaRN RoPE: extends rope_rotate_half with frequency scaling for long-context.
 * Implements the YaRN formula from the paper:
 * - Low frequencies (< beta_slow): scaled by 1/factor
 * - High frequencies (> beta_fast): no scaling
 * - Mid frequencies: linear interpolation between 1/factor and 1
 * - Partial rotary: only rotate the first rope_pct * head_dim dimensions
 */
static void rope_rotate_yarn(float *x, int pos, float theta, int hd, float factor, float rope_pct, float beta_slow, float beta_fast){
    int h2=hd/2; float tmp[512];
    if((size_t)hd>sizeof(tmp)/sizeof(float)){ fprintf(stderr,"head_dim too large for YaRN\n"); exit(1); }
    memcpy(tmp,x,(size_t)hd*sizeof(float));
    
    // Compute the number of dimensions to rotate (partial rotary)
    int rot_dim = (int)(hd * rope_pct);
    int rot_h2 = rot_dim / 2;
    
    for(int j=0;j<rot_h2;j++){
        // Base frequency for this dimension
        float inv_freq = powf(theta, -2.0f*(float)j/(float)hd);
        
        // YaRN scaling: compute the scale factor for this frequency
        float scale;
        if(inv_freq < beta_slow) {
            // Low frequency: scale by 1/factor
            scale = 1.0f / factor;
        } else if(inv_freq > beta_fast) {
            // High frequency: no scaling
            scale = 1.0f;
        } else {
            // Mid frequency: linear interpolation
            float t = (inv_freq - beta_slow) / (beta_fast - beta_slow);
            scale = (1.0f - t) * (1.0f / factor) + t * 1.0f;
        }
        
        // Apply the scaled frequency
        float scaled_inv_freq = inv_freq * scale;
        float ang = pos * scaled_inv_freq;
        float c = cosf(ang), s = sinf(ang);
        
        // Apply rotation
        float x1 = tmp[j], x2 = tmp[j + rot_h2];
        x[j] = x1 * c - x2 * s;
        x[j + rot_h2] = x2 * c + x1 * s;
    }
    
    // Copy the unrotated dimensions (if partial rotary)
    if(rot_dim < hd) {
        for(int j=rot_dim; j < hd; j++) {
            x[j] = tmp[j];
        }
    }
}
...
static void load_cfg(Cfg *c, const char *snap){
    ...
    // For Laguna, num_attention_heads is a list; use first element as default n_heads
    jval *hn = json_get(r, "num_attention_heads");
    if(hn && hn->t == J_ARR && hn->len > 0) c->n_heads = (int)hn->kids[0]->num;
    ...
    jval *rn=json_get(r,"norm_topk_prob"); c->route_norm=(rn&&rn->t==J_BOOL)?rn->boolean:1;
    c->router_scale=gf(r,"moe_routed_scaling_factor",1.f);
    ...
    const char *model_type_str = mt ? mt->str : NULL;
    if(model_type_str && strcasecmp(model_type_str, "laguna")==0) {
        c->model_type = 2; // LAGUNA
    } else if(model_type_str && strcasecmp(model_type_str, "hy_v3")==0) {
        c->model_type = 1; // HY3
    } else {
        c->model_type = 0; // GLM (default)
    }
    
    // Initialize Laguna-specific fields
    c->head_counts = NULL;
    c->kv_head_counts = NULL;
    c->layer_types = NULL;
    c->rope_theta_global = 500000.0f; // Default for Laguna global attention
    c->rope_theta_local = 10000.0f;   // Default for Laguna local attention
    c->rope_factor = 128.0f;          // YaRN factor
    c->rope_pct = 0.5f;               // Partial rotary for global layers
    c->yarn_beta_slow = 1.0f;         // YaNE beta_slow
    c->yarn_beta_fast = 32.0f;        // YaNE beta_fast
    c->attention_factor = 1.0f;       // YaNE attention scaling
    c->router_scaling_factor = 2.5f;  // For softplus per-head router
    
    if(c->model_type == 2) { // LAGUNA
        // Allocate arrays for per-layer configuration
        c->head_counts = malloc(c->n_layers * sizeof(int));
        c->kv_head_counts = malloc(c->n_layers * sizeof(int));
        c->layer_types = malloc(c->n_layers * sizeof(char));
        c->mlp_layer_types = malloc(c->n_layers * sizeof(char));
        
        // Read per-layer head counts from the config's num_attention_heads_per_layer array
        jval *heads_arr = json_get(r, "num_attention_heads_per_layer");
        if(!heads_arr) heads_arr = json_get(r, "num_attention_heads");
        for(int i=0; i<c->n_layers; i++) {
            if(heads_arr && heads_arr->t == J_ARR && i < heads_arr->len) {
                c->head_counts[i] = (int)heads_arr->kids[i]->num;
            } else {
                // Fallback: interpolate between 48 and 72
                c->head_counts[i] = 48 + (int)((float)i / (c->n_layers-1) * (72-48));
            }
            
            // KV heads are fixed at 8 for Laguna
            c->kv_head_counts[i] = 8;
            
            // Read layer_types (attention type: global vs sliding_window)
            jval *lt_arr = json_get(r, "layer_types");
            if(lt_arr && lt_arr->t == J_ARR && i < lt_arr->len && lt_arr->kids[i]->t == J_STR) {
                const char *lt = lt_arr->kids[i]->str;
                c->layer_types[i] = (strstr(lt, "sliding") != NULL) ? 1 : 0;
            } else {
                c->layer_types[i] = (i < c->n_layers / 4) ? 0 : 1;
            }
            
            // Read mlp_layer_types (dense vs sparse/MoE)
            jval *mlt_arr = json_get(r, "mlp_layer_types");
            if(mlt_arr && mlt_arr->t == J_ARR && i < mlt_arr->len && mlt_arr->kids[i]->t == J_STR) {
                const char *mlt = mlt_arr->kids[i]->str;
                c->mlp_layer_types[i] = (strstr(mlt, "sparse") != NULL) ? 1 : 0;
            } else {
                c->mlp_layer_types[i] = (i == 0) ? 0 : 1; // Layer 0 dense, rest sparse
            }
        }
        
        // Read Laguna-specific RoPE parameters if available
        jval *rope_cfg = json_get(r, "rope_parameters");
        if(rope_cfg && rope_cfg->t == J_OBJ) {
            jval *fa = json_get(rope_cfg, "full_attention");
            jval *sa = json_get(rope_cfg, "sliding_attention");
            if(fa && fa->t == J_OBJ) {
                jval *theta_g = json_get(fa, "rope_theta");
                jval *factor = json_get(fa, "factor");
                jval *pct = json_get(fa, "partial_rotary_factor");
                jval *bs = json_get(fa, "beta_slow");
                jval *bf = json_get(fa, "beta_fast");
                jval *af = json_get(fa, "attention_factor");
                if(theta_g) c->rope_theta_global = (float)theta_g->num;
                if(factor) c->rope_factor = (float)factor->num;
                if(pct) c->rope_pct = (float)pct->num;
                if(bs) c->yarn_beta_slow = (float)bs->num;
                if(bf) c->yarn_beta_fast = (float)bf->num;
                if(af) c->attention_factor = (float)af->num;
            }
            if(sa && sa->t == J_OBJ) {
                jval *theta_l = json_get(sa, "rope_theta");
                if(theta_l) c->rope_theta_local = (float)theta_l->num;
            }
        }
        
        // Read router scaling factor if available
        jval *router_scale = json_get(r, "router_scaling_factor");
        if(router_scale) c->router_scaling_factor = (float)router_scale->num;
    }
...
static float *ld(Model *m, const char *name){
 ...
 static float *ld_first3(Model *m, const char *a, const char *b, const char *c){
     if(st_has(&m->S,a)) return ld(m,a);
     if(b&&st_has(&m->S,b)) return ld(m,b);
     if(c&&st_has(&m->S,c)) return ld(m,c);
     fprintf(stderr,"missing %s (also tried %s, %s)\n",a,b?b:"",c?c:""); exit(1);
     return NULL;
 }
...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits){
    ...
    Cfg *c=&m->c; char nm[512], nm2[512], nm3[512]; int D=c->hidden, hd=c->head_dim;
    int qo=c->n_heads*hd, kvo=c->n_kv_heads*hd;
    int io_bits=8;  // embed/lm_head always int8 (matches converter default)
...
for(int i=0;i<c->n_layers;i++){
        ...
        // Laguna-specific: use per-layer head counts
        int qo, kvo;
        if(c->model_type == 2) { // LAGUNA
            qo = c->head_counts[i] * c->head_dim;
            kvo = c->kv_head_counts[i] * c->head_dim;
        } else {
            qo = c->n_heads * c->head_dim;
            kvo = c->n_kv_heads * c->head_dim;
        }
        ...
        // Load g_proj (per-head attention gate) if available
        l->g.qf = NULL; l->g.q8 = NULL; l->g.q4 = NULL; l->g.s = NULL;
        if(st_has(&m->S, P("self_attn.g_proj.weight"))) {
            int g_heads = (c->model_type == 2) ? c->head_counts[i] : c->n_heads;
            l->g = qt_load(m, P("self_attn.g_proj.weight"), g_heads, D, dbits);
        }
        
        // Laguna-specific: determine sparsity based on mlp_layer_types
        int is_sparse;
        if(c->model_type == 2) { // LAGUNA
            is_sparse = (c->mlp_layer_types[i] == 1); // 1 = sparse (MoE)
        } else {
            is_sparse = (i>=c->first_dense);
        }
        l->sparse=is_sparse;
        ...
        if(!l->sparse){
            ...
            snprintf(nm3,sizeof(nm3),"model.layers.%d.mlp.experts.e_score_correction_bias",i);
            l->router_bias=ld_first3(m,nm2,nm,nm3);
    ...
    /* Allocate expert cache for sparse layers (was only done inside MTP block — bug) */
    for(int i=0;i<c->n_layers;i++){
        if(m->L[i].sparse && !m->ecache[i])
            m->ecache[i]=calloc(m->ecap,sizeof(ESlot));
    }
...
static void kv_alloc(Model *m, int max_t){
    ...
    // For Laguna models, we need to handle per-layer KV head counts
    if(c->model_type == 2) { // LAGUNA
        if(g_kv_i8){
            ...
            for(int i=0;i<NR;i++){
                int hd=c->head_dim;
                int kv_heads = (i < c->n_layers) ? c->kv_head_counts[i] : c->n_kv_heads; // Use per-layer for real layers, global for MTP
                int64_t slot=(int64_t)kv_heads*max_t*hd;
                int64_t sc_slot=(int64_t)kv_heads*max_t;
    ...
    } else { // Original logic for GLM/Hy3
        if(g_kv_i8){
            m->Kq=calloc(NR,sizeof(int8_t*)); m->Vq=calloc(NR,sizeof(int8_t*));
            m->Kscale=calloc(NR,sizeof(float*)); m->Vscale=calloc(NR,sizeof(float*));
            for(int i=0;i<NR;i++){
...
static void attention(Model *m, Layer *l, int layer, float *x, int S, int pos_base, float *out){
    ...
    // Laguna-specific: use per-layer head counts if this is a Laguna model
    if(c->model_type == 2) { // LAGUNA
        H = c->head_counts[layer];
        Hkv = c->kv_head_counts[layer];
        hd = c->head_dim;
    } else {
        H = c->n_heads;
        Hkv = c->n_kv_heads;
    ...
    // Apply YaNE attention_factor for global attention layers
    if(c->model_type == 2 && c->layer_types && c->layer_types[layer] == 0) {
        scale *= c->attention_factor;
    }
    double ta0=now_s();
    ...
    // Laguna-specific: compute per-head attention gate (g_proj)
    // Gate is applied to attention output BEFORE o_proj, not to query
    float *G=NULL;
    if(c->model_type == 2 && l->g.O > 0) {
        G=falloc((int64_t)S*H);
        matmul_qt(G,x,&l->g,S);
    }
    
    // Laguna-specific RoPE parameters
    float theta = c->theta; // Default
    float rope_factor = 1.0f; // No YaRN by default
    float rope_pct = 1.0f; // Full rotary by default
    
    if(c->model_type == 2) { // LAGUNA
        // Determine if this layer uses global or sliding window attention
        int is_sliding = (c->layer_types && c->layer_types[layer] == 1); // 1 = sliding_window
        
        if(is_sliding) {
            // Sliding window attention layers
            theta = c->rope_theta_local; // θ=10,000 for sliding layers
            rope_factor = 1.0f; // No YaRN for sliding layers
            rope_pct = 1.0f; // Full rotary for sliding layers
        } else {
            // Global attention layers
            theta = c->rope_theta_global; // θ=500,000 for global layers
            rope_factor = c->rope_factor; // YaRN factor 128
            rope_pct = c->rope_pct; // Partial rotary 0.5 for global layers
        }
    }
...
for(int s=0;s<S;s++){
        ...
        for(int h=0;h<H;h++){
            ...
            // Apply Laguna-specific RoPE
            if(rope_factor != 1.0f) {
                // Use YaRN RoPE for global attention layers
                rope_rotate_yarn(qh, pos, theta, hd, rope_factor, rope_pct, c->yarn_beta_slow, c->yarn_beta_fast);
            } else {
                // Use standard RoPE for sliding window layers
                int effective_hd = hd;
                if(rope_pct < 1.0f) {
                    effective_hd = (int)(hd * rope_pct);
                }
                rope_rotate_half(qh, pos, theta, effective_hd);
            }
        }
        for(int kh=0;kh<Hkv;kh++){
            ...
            // Apply Laguna-specific RoPE to keys
            if(rope_factor != 1.0f) {
                // Use YaRN RoPE for global attention layers
                rope_rotate_yarn(kk, pos, theta, hd, rope_factor, rope_pct, c->yarn_beta_slow, c->yarn_beta_fast);
            } else {
                // Use standard RoPE for sliding window layers
                int effective_hd = hd;
                if(rope_pct < 1.0f) {
                ...
                rope_rotate_half(kk, pos, theta, effective_hd);
            }
            if(g_kv_i8){
    ...
    for(int s=0;s<S;s++) for(int h=0;h<H;h++){
        ...
        for(int jj=0;jj<nt;jj++){
            ...
            // Laguna-specific sliding window mask
            bool skip_attention = false;
            if(c->model_type == 2 && c->layer_types && c->layer_types[layer] == 1) {
                // Sliding window attention: only attend to tokens within window size
                int window_size = 512; // From Laguna config
                if(pos - t > window_size) {
                    skip_attention = true;
                }
            }
            
            if(skip_attention) {
                sc[nv] = -1e30f; // Mask out attention to tokens outside window
            } else {
                if(g_kv_i8){
                    if(t==pos){
        ...
        for(int jj=0;jj<nt;jj++){
            ...
            // Apply the same sliding window mask to value selection
            ...
            if(skip_value) {
                // Skip adding zero vector (already zeroed)
            } else {
                if(g_kv_i8){
                    if(t==pos){
    ...
    // Apply g_proj gate to attention output (before o_proj)
    if(G) {
        for(int s=0;s<S;s++) for(int h=0;h<H;h++){
            float gate = log1pf(expf(G[(int64_t)s*H+h])); // softplus
            for(int d=0;d<hd;d++) ctx[(int64_t)s*H*hd+(int64_t)h*hd+d] *= gate;
        }
        free(G);
    }
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out){
    ...
    for(int s=0;s<S;s++){
        ...
        matmul(logit,xs,l->router,1,D,E);           // Raw logits from router
        for(int e=0;e<E;e++) choice[e]=l->router_bias[e];     // Bias term (per-expert, not matmul)
        for(int e=0;e<E;e++){
            if(c->model_type == 2) { // LAGUNA: Use softplus per-head router
                // Laguna router: per-head softplus + routed_scaling_factor
                float combined = logit[e] + choice[e];  // Raw logit + bias
                // Softplus: ln(1 + exp(x))
                float activated = log1pf(expf(combined));  // More numerically stable
                // Store activated value for both selection and weighting
                // (final routing scaling is applied later in the common code)
                choice[e] = activated;
                logit[e] = activated;
            } else {
                // Original behavior: sigmoid + bias
                logit[e] = sigmoidf(logit[e]);
                choice[e] = logit[e];  // Already has bias added
            }
        }
